...
static cmd_mgr_status_t walk_handler(int diraction, int arg_2);
...
cmd_mgr_status_t cmd_mgr_process(void)
{
	...
	if (task_executed)
	{
		...
		if (task_ctx->task_stage == TASK_STAGE_IDLE) // task execution finished
		{
			allow_position_control(true);
...
static cmd_mgr_status_t change_height_handler(int h_delta, int arg_2)
{
	...
	switch (task_ctx->task_stage){
		case TASK_STAGE_1:
			...
			{
				task_ctx->change_height.h_delta = (h_delta == CMD_PARAM_OMITTED) ? DEFAULT_HEIGHT : h_delta;
				task_ctx->task_stage = TASK_STAGE_2;
			}
			else
			{
...
static cmd_mgr_status_t fixed_turn_handler(int angle, int arg_2)
{
	...
	switch (task_ctx->task_stage){
		...
		case TASK_STAGE_3: // Lift 3 legs
		{
			...
			else if (SQR(L1 + L2) > SQR(pos_mgr_get_max_legs_height()) + SQR(task_ctx->fixed_turn.new_r))
			{
		...
		break;
		case TASK_STAGE_6: // start resting to stabilyze
			task_ctx->fixed_turn.rest_end = HAL_GetTick() + REST_BETWEEN_STEPS;
			task_ctx->task_stage = TASK_STAGE_7;
		break;
		case TASK_STAGE_7: // wait rest end
			if (task_ctx->fixed_turn.rest_end < HAL_GetTick())
			{
		...
		break;
		case TASK_STAGE_9: // Turn all legs to default position
			...
			{
				task_ctx->task_stage = TASK_STAGE_10;
			}
			else 
			{
				task_ctx->task_stage = TASK_STAGE_4;
...
static cmd_mgr_status_t walk_handler(int direction, int arg_2)
{
	cmd_mgr_status_t status = CMD_MGR_SUCCESS;
	...
	switch (task_ctx->task_stage){
		...
		if ((direction == CMD_PARAM_OMITTED) || ((direction >= -180) && (direction <= 180)))
		{
			...
			else
			{
				...
				// Init task context
				if (direction == CMD_PARAM_OMITTED)
				{
					task_ctx->walk.direction = 0;
					task_ctx->walk.wandering = true;
				}
				else
				{
					task_ctx->walk.direction = direction;
					task_ctx->walk.wandering = false;
				}
				task_ctx->walk.legs_group = 0;
      }
		}	
		else
		{
		...
		case TASK_STAGE_3: // Lift 3 legs
		{
			...
			else if (pos_mgr_lift_three_legs(task_ctx->walk.legs_group) == POS_MGR_SUCCESS)
			{
				task_ctx->task_stage = TASK_STAGE_4;
			}
			else
		...
		break;
		case TASK_STAGE_4: // Turn all legs
		{
			pos_mgr_status_t sub_status = POS_MGR_SUCCESS;
			uint16_t r[LEGS_COUNT];
			uint16_t angle[LEGS_COUNT];
			
			for (uint8_t leg_id = task_ctx->walk.legs_group; leg_id < LEGS_COUNT && sub_status == POS_MGR_SUCCESS; leg_id += 2)
			{
				sub_status = pos_mgr_prepare_leg_step(leg_id, task_ctx->walk.direction, r + leg_id, angle + leg_id);
			}
			for (uint8_t leg_id = 1 - task_ctx->walk.legs_group; leg_id < LEGS_COUNT && sub_status == POS_MGR_SUCCESS; leg_id += 2)
			{
				sub_status = pos_mgr_prepare_leg_step(leg_id, 180 + task_ctx->walk.direction, r + leg_id, angle + leg_id);
			}
			...
			if (sub_status == POS_MGR_SUCCESS)
			{
				...
				{
					pos_mgr_set_leg_position(leg_id, CMD_PARAM_OMITTED, r[leg_id], angle[leg_id]);
				}
				task_ctx->task_stage = TASK_STAGE_5;
			}
			else
		...
		{
			bool done;
			if (pos_mgr_fall_three_legs(task_ctx->walk.legs_group, &done) == POS_MGR_SUCCESS)
			{
				if (done)
				{
					task_ctx->walk.legs_group = 1 - task_ctx->walk.legs_group;
					task_ctx->task_stage = TASK_STAGE_6;
					allow_position_control(true);
				}
			}
		...
		break;
		case TASK_STAGE_6: // start resting to stabileze
			task_ctx->walk.rest_end = HAL_GetTick() + REST_BETWEEN_STEPS;
			task_ctx->task_stage = TASK_STAGE_7;
		break;
		case TASK_STAGE_7: // wait rest end
			if (task_ctx->walk.rest_end < HAL_GetTick())
			{
				task_ctx->task_stage = TASK_STAGE_8;
			}			
		break;
		...
		{
			allow_position_control(false);
			if (pos_mgr_lift_three_legs(task_ctx->walk.legs_group) == POS_MGR_SUCCESS)
			{
				if (abort_active_command)
				{
					task_ctx->task_stage = TASK_STAGE_9;
		...
		break;
		case TASK_STAGE_9: // get default position (3 legs are on surface)
			for (uint8_t leg_id = 0; leg_id < LEGS_COUNT; ++leg_id)
			{
			...
			task_ctx->task_stage = TASK_STAGE_10;
		break;
		case TASK_STAGE_10: // Fall 3 legs
		{
			bool done;
			if (pos_mgr_fall_three_legs(task_ctx->walk.legs_group, &done) == POS_MGR_SUCCESS)
			{
