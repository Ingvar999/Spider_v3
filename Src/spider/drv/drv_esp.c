...
#define ESP_TX_DATA_BUF_QUEUE_SIZE				(3)
...
static volatile esp_state_t esp_state = ESP_RESET;
...
static bool handle_event(const char* input)
{
	bool handled = true;
	...
	if (strlen(input) > 3)
	{
		...
		else if (str_starts_with(input + 2, "CONNECT")) 
		{
			...
			if ((active_connection == INVALID_CONNECTION_ID) || (active_connection == connection))
			{
				...
				LOG_INFO("New connection %c", connection);
			}
			else
			{
				LOG_WARN("Refuse connection %c", connection);
...
void drv_esp_send(const char *data, int data_len)
{
	...
	else
	{
		...
		else
		{
			...
			{
				LOG_DBG("Postpone esp send, state %d", esp_state);
			}
...
void drv_esp_handle_input(const char *input)
{
	if (input[0] != '\r')
	{
		...
		{
			LOG_DBG("ESP state %d, msg: %s", esp_state, input);
		}
		...
		if (!handle_event(input))
		{
			...
			do {
				...
				switch (esp_state)
				{
					...
					case ESP_HOME_AP_INIT_3:
						if (str_starts_with(input, "WIFI CONNECTED") || str_starts_with(input, "WIFI GOT IP"))
						{
							handled = true;
						}
						else if (is_ok)
			...
			if (!handled)
			{
				LOG_WARN("Unhandled ESP state %d, msg: %s", esp_state, input);
			}
		}
